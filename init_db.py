...
if __name__ == '__main__':
    ...
    db_file = "taxi_api.dao.%s.base.DBBaseDao" % cfg["api"]["database"]
    ...
    _dao_path = cur_dir + "/taxi_api/dao/" + cfg["api"]["database"]
    ...
    for file_in_dir in os.listdir(_dao_path):
        if file_in_dir.endswith(".py") and not file_in_dir in _ignore_in_load:
            ...
            clazz = Helpers.get_class(
                "taxi_api.dao.%s.%s.%s" % (cfg["api"]["database"], module_name, dao_class_name))
